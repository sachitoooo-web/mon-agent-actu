...
# 2. INSTRUCTIONS DE L'AGENT (C'est ici qu'il "apprend" qui il est)
# Tu pourras modifier ce texte plus tard pour lui apprendre de nouvelles choses.
SYSTEM_PROMPT = """
...
- Si l'utilisateur te donne une nouvelle info sur lui ou sur un sujet, note-la.
"""

# 3. GESTION DES CLÉS SECRÈTES (Sécurité)
try:
...
def ask_agent(user_message):
    # On prépare le modèle Gemini
    llm = ChatGoogleGenerativeAI(model="gemini-1.5-flash", google_api_key=GOOGLE_API_KEY)
    # On prépare l'outil de recherche
    search_tool = TavilySearchResults(tavily_api_key=TAVILY_API_KEY, k=3)
    
    context_web = ""
    # Si on demande de l'actu, on active la recherche
    ...
    if any(mot in user_message.lower() for mot in mots_cles):
        with st.status("🔍 Je scanne le web...", expanded=True) as status:
            try:
                ...
                st.warning("La recherche web a échoué, je réponds avec mes connaissances de base.")
    ...
    # On ajoute le message actuel + les infos trouvées sur le web
